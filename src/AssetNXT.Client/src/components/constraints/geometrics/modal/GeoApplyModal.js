...
export default class DeleteConstrains extends Component {
  ...
  componentWillReceiveProps() {
    if (this.props.geo !== null) {
      this.setState({
        geo: this.props.geo,
        setDevices: this.props.geo.devices,
      });
    }
  }
  ...
  renderSearchResults() {
    const listItems = this.state.filteredDevices.map((device, i) => {
      if (i < 5 && !this.state.setDevices.includes(device.deviceId)) {
        return (
          <Row className="mt-1">
            <Col key={device.deviceId} xs="9">
              {device.deviceId}
            </Col>
            <Col xs="3" className="d-flex justify-content-end">
              <Button
                onClick={() => this.applyData(device)}
                size="sm"
                color="info"
              >
                Apply
        ...
        <Row className="mt-1">
          ...
          <Col xs="3" className="d-flex justify-content-end">
            <Button
              color="link"
              size="sm"
              onClick={() => this.deleteData(deviceId)}
  ...
  changeFilteredDevices(input) {
    let filtered = [];
    if (input !== "")
      filtered = this.state.devices.filter((device) =>
        device.deviceId.toLowerCase().includes(input.toLowerCase())
      );
